  ...
  function start (selector, startOpts) {
    ...
    if (!selector) {
      ...
      onReady(function onReady () {
        ...
        if(isPromise(newTree)) {
          newTree.then(newDom => _rootNode = yo.update(oldTree, newDom))
        } else {
  ...
  function render (data, state, prev, name, createSend) {
    if (!_frame) {
      _frame = nanoraf(function (state, prev) {
        ...
        if(isPromise(newTree)) {
          newTree.then(newDom => _rootNode = yo.update(_rootNode, newDom))
        } else {
